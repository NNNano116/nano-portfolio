...
// 이력(2페이지) 카드 — hover/focus 시 상세가 펼쳐진다(현재는 placeholder).
const RESUME: { key: string; lines: string[] }[] = [
  { key: '내 상태', lines: ['현재 상태·한 줄 소개', '(추후 채움)'] },
  { key: '학력', lines: ['학교 / 전공 / 기간', '(추후 채움)'] },
  { key: '교육', lines: ['부트캠프 · 수료 과정', '(추후 채움)'] },
  { key: '경력', lines: ['회사 / 역할 / 기간', '(추후 채움)'] },
]
          ...
          <div className="resume-grid">
            {RESUME.map((c) => (
              <article className="card" key={c.key} tabIndex={0}>
                ...
                <span className="card__more" aria-hidden="true">＋</span>
              </article>
            ))}
          </div>
        </div>
